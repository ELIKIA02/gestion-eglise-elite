...
import { LiturgicalTheme } from '../types';
import { Bookmark, Plus, Edit2, Trash2, ChevronLeft, ChevronRight, X, Search } from 'lucide-react';
...
export default function LiturgicalThemesModule() {
  ...
  const [formThemeType, setFormThemeType] = useState<LiturgicalTheme['themeType']>('dimanche');
  const [formSeason, setFormSeason] = useState<LiturgicalTheme['season']>(undefined);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(ref, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        data.push({ id: doc.id, ...doc.data() } as LiturgicalTheme);
      });
      setThemes(data);
      setLoading(false);
    }, (err) => {
  ...
  const filteredThemes = useMemo(() => {
    return themes.filter(t => {
      const themeMonth = new Date(t.date + 'T00:00:00').getMonth();
      const themeYear = new Date(t.date + 'T00:00:00').getFullYear();
      const matchesMonth = themeMonth === currentMonth.getMonth() && themeYear === currentMonth.getFullYear();
  ...
  const openAdd = () => {
    ...
    setFormBibleText('');
    setFormDescription('');
    setFormThemeType('dimanche');
    setFormSeason(undefined);
    setFormHymns('');
    setShowModal(true);
  };
  ...
  const openEdit = (theme: LiturgicalTheme) => {
    ...
    setFormDate(theme.date);
    setFormPreacher(theme.preacher || '');
    setFormBibleText(theme.bibleText || '');
    setFormDescription(theme.description || '');
    setFormThemeType(theme.themeType);
    setFormSeason(theme.season);
    setFormHymns(theme.hymns || '');
    setShowModal(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const data: any = {
        ...
        bibleText: formBibleText.trim(),
        description: formDescription.trim(),
        themeType: formThemeType,
        season: formSeason || null,
        hymns: formHymns.trim(),
      };
      if (editingTheme) {
      ...
      handleFirestoreError(err, editingTheme ? OperationType.UPDATE : OperationType.CREATE, COLLECTION);
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs">
          ...
          <select value={filterType} onChange={e => setFilterType(e.target.value)}
            ...
            {Object.entries(themeTypeLabels).map(([k, v]) => (
              <option key={k} value={k}>{v}</option>
            ))}
          </select>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between p-3 border-b border-slate-100">
          <button onClick={prevMonth}
            className="p-1.5 hover:bg-slate-100 rounded-lg transition-all cursor-pointer text-slate-500">
        ...
        <div className="grid grid-cols-7 border-b border-slate-100">
          {weekdayHeaders.map(day => (
            <div key={day} className="text-center text-[10px] font-semibold text-slate-400 py-2 uppercase tracking-wider">
              {day}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7">
          {calendarDays.map((cell, idx) => {
            if (!cell) {
              return <div key={`empty-${idx}`} className="min-h-[100px] p-1 bg-slate-50/50" />;
            }
            ...
            return (
              <div key={dateStr}
                className={`min-h-[100px] p-1.5 border-b border-r border-slate-100 text-left transition-all relative ${
                ...
                <div className="space-y-1">
                  {dayThemes.slice(0, 2).map(theme => (
                    <div key={theme.id}
                      className="text-[9px] leading-tight px-1 py-0.5 rounded bg-indigo-50 text-indigo-700 font-semibold truncate cursor-pointer hover:bg-indigo-100 transition-colors"
                      title={theme.title}
                  ...
                  {dayThemes.length > 2 && (
                    <span className="text-[9px] text-indigo-500 font-semibold pl-1">+{dayThemes.length - 2}</span>
                  )}
                </div>
              </div>
            );
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold border uppercase ${
                        seasonColors[theme.season || ''] || 'bg-slate-100 text-slate-600 border-slate-200'
                      }`}>
                        {seasonLabels[theme.season || ''] || '—'}
                      </span>
                      <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 uppercase">
                        {themeTypeLabels[theme.themeType] || theme.themeType}
                      </span>
                    </div>
                    ...
                    {theme.description && (
                      <p className="text-[10px] text-slate-400 mt-1 line-clamp-2">{theme.description}</p>
                    )}
                    {theme.hymns && (
                      <p className="text-[10px] text-slate-400 mt-0.5">Cantiques : {theme.hymns}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button onClick={() => handleDelete(theme.id!)}
                      className="p-1.5 rounded-lg border border-red-200 hover:bg-red-50 cursor-pointer"
                      title="Supprimer">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              <h3 className="text-sm font-bold text-slate-800">
                {editingTheme ? 'Modifier le thème' : 'Nouveau thème liturgique'}
              </h3>
            ...
            <form onSubmit={handleSave} className="p-4 space-y-3">
              <div className="space-y-1">
                ...
                <input type="text" value={formTitle} onChange={e => setFormTitle(e.target.value)}
                  ...
                  className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:outline-indigo-600 bg-white" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input type="text" value={formPreacher} onChange={e => setFormPreacher(e.target.value)}
              ...
              <div className="space-y-1">
                ...
                <input type="text" value={formBibleText} onChange={e => setFormBibleText(e.target.value)}
              ...
              <div className="space-y-1">
                ...
                <textarea value={formDescription} onChange={e => setFormDescription(e.target.value)}
                  ...
                  className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:outline-indigo-600 bg-white resize-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-600">Type *</label>
                  <select value={formThemeType} onChange={e => setFormThemeType(e.target.value as LiturgicalTheme['themeType'])}
                    className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:outline-indigo-600 bg-white">
                    {Object.entries(themeTypeLabels).map(([k, v]) => (
                      <option key={k} value={k}>{v}</option>
                    ))}
                ...
                <div className="space-y-1">
                  ...
                  <select value={formSeason || ''} onChange={e => setFormSeason((e.target.value || undefined) as LiturgicalTheme['season'])}
                    className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:outline-indigo-600 bg-white">
              ...
              <div className="space-y-1">
                ...
                <input type="text" value={formHymns} onChange={e => setFormHymns(e.target.value)}
                  placeholder="Ex: ADC 45, 120"
                  className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:outline-indigo-600 bg-white" />
              </div>
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
                <button type="button" onClick={closeModal}
                  className="text-xs font-semibold text-slate-500 hover:bg-slate-50 px-4 py-2 rounded-lg cursor-pointer transition-all">
                  Annuler
                </button>
